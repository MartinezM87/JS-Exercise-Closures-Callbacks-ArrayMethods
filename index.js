...
function processSum(numberList, callback) {
  return callback(numberList.reduce((acc, curVal) => 
  {
    return acc + curVal;
  }, 0));
}
...
function processDuplicateFree(list, callback) {
  return callback(list.filter((cVal, index, arr) => arr.indexOf(cVal) == index));
}
...
function getFullNames(arr) {
  ...
  arr.forEach(cVal => 
    {
    newArr.push(`${cVal.last_name}, ${cVal.first_name}`);
  });
  return newArr;
}
...
function firstNamesAllCaps(arr) {
  return arr.map(cVal => cVal.first_name.toUpperCase());
}
...
function getRunnersByTShirtSize(arr, tShirtSize) {
  return arr.filter(cVal => 
    {
    return cVal.shirt_size == tShirtSize;
  });
}
...
function tallyUpDonations(arr) {
  return arr.reduce((acc, cVal) => acc + cVal.donation, 0);
}
